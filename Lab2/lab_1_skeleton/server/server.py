...
try:
    ...
    def send_id(req = 'POST'):
        ...
        # Add own ID information to the current payload
        my_payload = {'{}'.format(node_id):my_id}
        payload.update(my_payload)

        print "The new payload is:", payload

        path = "/propagate/elect/" + str(node_id+1 % len(vessel_list))

        print "node_id:", node_id

        for vessel_id, vessel_ip in vessel_list.items():
            if int(vessel_id) == node_id % (len(vessel_list)-1) + 1:     # This is the next neighbor
                success = contact_vessel(vessel_ip, path, payload, req)
                if not success:
    ...
    def elect_leader():
        global node_id, my_id, leader_id, payload
        print "In function elect_leader(): payload =", payload

        if payload.has_key(node_id):
            leader_val = max(payload.values())
            for ip, val in payload.items():
                if val == leader_val:
                    leader_id = ip
        else:
            send_id('POST')
    ...
    def propagation_received(action, element_id):
        ...
        if action=="elect":
            payload = dict(request.body.read())
