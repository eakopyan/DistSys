...
try:
    ...
    def send_id(path, payload, req = 'POST'):
        global vessel_list, node_id

        # Pick a random number as ID
        my_id = randint(0,1000)
        # Add own ID information to the current payload
        my_payload = {'{}'.format(vessel_id):my_id}
        payload.update(my_payload)

        for vessel_id in vessel_list.items():
            if int(vessel_id) == node_id+1:     # This is the next neighbor
                success = contact_vessel(vessel_ip, path, payload, req)
                if not success:
                    print "\n\nCould not contact vessel {}\n\n".format(vessel_id)


    def elect_leader(payload):
        global vessel_list, node_id

        if payload.has_key(node_id): # The loop is full, time to choose
